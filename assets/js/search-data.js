var store = [{
        ...
        "title": "传感器颜色调制 (二)",
        "excerpt":"通过coded-aperture 和 dispenser进行颜色调制 coded-aperture的优点 有利于deblur PSF是scene到image的kernel: $\\text{image}=PSF*\\text{scene}$ 如果对image做deblur, $\\text{scene}=F^{-1}(\\frac{F(\\text{image})}{F(PSF)})=F^{-1}(\\frac{F(\\text{image})}{MTF})$ 传统的MTF存在cutoff点, 即在cutoff之上的的高频部分是0, 而coded-aperture可以保留部分高频信息, 使MTF没有完全为0的点 我不能确定coded aperture是否必要, 看起来颜色调制主要还是通过dispenser完成的CASSI coded_apeture+dispenser 仿真数据： 1 CASSI: coded_apeture+dispenser coded_apeture把mask应用于所有通道 dispenser依次给mask的通道图像y-axis 2pixel的shift 所有通道叠加(一个像素是同行28x2=56个不同通道像素的叠加，出来的图像是很模糊的) 仿真通过shot noise: $Y_{sim}=B(Y/QE,QE)$ 本来读数是Y, qe是quantum efficiency B是binomial 分布 这段其实没懂为什么要用binomial分布，而不是poisson分布$P(Y)$ , 大概Y/QE是平均光子数，所以sample这么多次，每次都有qe的概率转换为电子；但是仍然不理解为什么要在光电转换的部分随机 仿真: 实际拍摄: recon: 看起来仿真和实际拍摄的情况并不太一样啊?仿真的y方向距离更大 数据集使用： simulation: 训练： data: cave_1024_28 crop_size: 随机crop到256x256, 和mask做simulation shift3d mask:...","categories": ["docs","algorithm"],
        "tags": ["content","sensor"],
