...
tags:
  ...
  - vue
  - worldquant
last_modified_at: 2025-12-14T23:46
...
4. 支持中途暂停任务执行
	...
	3. 在代码中循环(或监听)数据库的更新, 适应随时插入priority任务.
...
* **分页问题**：`naive-ui` 的 `data-table` 不会记录当前页
  ...
  * 最后我还是自己管理了分页, 因为不想一次性加载全部数据库. 并且把query, 排序都移到后端处理.
...
* **css样式问题**：
  ...
  * 在调整样式时, 用调试功能`inspect element`确认组件的真实名称. 不要让ai从代码猜测.
...
* **日志调试**：打印完整log用于调试非常重要, 尤其调用外部脚本时, 统一调用函数, 把log打印写在里面.
	* 用async函数和tokio 库设置stdout, stderr实时刷新
...
* **使用事件循环运行带异常捕获的主逻辑**(`asyncio.get_event_loop()` 和 `traceback.format_exc()`), 输出定位到错误代码级别的log.

### release部分

- 把配置的逻辑全部从代码中抽出, 写在配置文件里. 用设置界面显示和修改配置文件
- build时给用户生成一个resource, 把配置文件放在里面, 让用户可以不通过源码更改配置
