...
tags:
  ...
  - python
  - dsl
last_modified_at: 2025-09-12T17:50
...
可以把字符串token解析成**AST 的构建过程**理解成“对运算符按优先级排序”的过程.

| 方法                                                                                                          | 优缺点                  | 类比排序思路   | 平均复杂度                        |
| ----------------------------------------------------------------------------------------------------------- | -------------------- | -------- | ---------------------------- |
| 用正则表达式替换三元运算符                                                                                               | 思路naive, 但是可读性差，难以维护 |          | O(n logn)+kO(n)<br>全字符串扫描+递归 |
| 设计语法树解析器（Parser）                                                                                            | 灵活性高, 但实现较复杂         | 选择排序     | O(n logn)<br>近似二分递归          |
| 结合 [Pratt Parser](https://www.google.com/search?client=safari&rls=en&q=pratt+parser&ie=UTF-8&oe=UTF-8) 优化解析 | 代码简洁, 可读性高, 扩展性好     | 插入排序/堆排序 | O(n)<br>线性扫描                 |
...
2. 具体流程: 
	1. **预处理（字符串替换/注释/关键字替换）**
		...
		- token 化
		- 自顶向下递归解析（赋值 → 三元→ 二元**按最低 precedence 为根** → 原子/函数/一元）
	2. **解析每句代码:**
		- 顶层赋值检测
			- 去掉最外层的分号
...
- [**replace_ternary.py**](https://gist.github.com/roshameow/01c0191fae3c534050fa6ab076640549#file-replace_ternary-py) 
-  [**parser.py**](https://gist.github.com/roshameow/01c0191fae3c534050fa6ab076640549#file-parser-py)
- [**pratt_parser.py**](https://gist.github.com/roshameow/01c0191fae3c534050fa6ab076640549#file-pratt_parser-py)
